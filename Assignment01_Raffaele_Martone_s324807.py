...
def checkPerclos(ears,earsWindow,image, img_w, img_h):
    ...
    cv2.rectangle(image, (x_start, y_start), 
        ...
        (255, 255, 255), -1)
    cv2.putText(image, "Perclos: {:.2f}".format(perclos), (50, 900), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0, 255), 2)
    return perclos

def checkAlert(statusIn10s, perclos, ears, image, img_w, img_h):
    x_start = int(img_w * 0.4)
    y_start = 60
    x_width = img_w // 3 
    y_height = 70

    # Draw a rectangle for the alert message
    cv2.rectangle(image, (x_start, y_start), 
                  (x_start + x_width, y_start + y_height), 
                  (255, 255, 255), -1)

    if st.mean(statusIn10s) > 0.8 and len(statusIn10s) > 300:
        cv2.putText(image, "DROWSY: Status > 80%", 
                    (x_start + 10, y_start + 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        if telegram_alert:
            asyncio.run(send_alert("ATTENZIONE: Il conducente sembra assonnato!"))
    elif perclos > 0.8:
        cv2.putText(image, "DROWSY: Perclos > 80%", 
                    (x_start + 10, y_start + 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        if telegram_alert:
            asyncio.run(send_alert("ATTENZIONE: Il conducente sembra assonnato!"))
    elif all(ear > 80 for ear in ears):
        cv2.putText(image, "DROWSY: Ears > 80%", 
                    (x_start + 10, y_start + 60), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
    else:
        cv2.putText(image, "Check for EARS&PERCLOS: AWAKE", 
                    (x_start + 10, y_start + 60), 
...
def check_driver_distraction(pitch_left_eye, yaw_left_eye, pitch_right_eye, yaw_right_eye, pitch, yaw, roll, image, img_w):
    ...
    if is_distracted:
        ...
        cv2.putText(image, f"Check for the Gaze&Head: Distracted: {int(distraction_duration)}s", (int(img_w * 0.4)+10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
    else:
        cv2.putText(image, f"Check for the Gaze&Head: Not Distracted", (int(img_w * 0.4)+10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255,0), 2)
        distraction_start_time = None
        alert_sent = False
...
def checkAwake(ear, ears, start, statusIn10s, image, img_w, img_h):
    ...
    cv2.putText(image, "Mean Status: {:.2f}".format(st.mean(statusIn10s)), (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0, 255), 2)
...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            ...
            perclos = checkPerclos(ears,earsWindow,image, img_w, img_h)
            checkAlert(statusIn10s,perclos, ears, image, img_w, img_h)
